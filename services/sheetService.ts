...
export const fetchGoogleSheetData = async (url: string): Promise<{ headers: string[], rows: SheetRow[] }> => {
  ...
  // Example: https://docs.google.com/spreadsheets/d/ID/edit#gid=0 -> https://docs.google.com/spreadsheets/d/ID/export?format=csv
  let csvUrl = url;
  if (url.includes('/edit')) {
    csvUrl = url.split('/edit')[0] + '/export?format=csv';
  } else if (!url.endsWith('/export?format=csv')) {
    // Attempting a guess if it's just the ID or partial URL
