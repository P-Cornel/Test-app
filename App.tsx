...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col transition-colors duration-300 bg-gray-50 dark:bg-slate-950 font-sans">
      <header className="bg-white dark:bg-slate-900 border-b dark:border-slate-800 px-6 py-4 flex items-center justify-between sticky top-0 z-[1000] shadow-sm transition-colors duration-300">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-indigo-600 to-blue-600 dark:from-indigo-400 dark:to-blue-400">SheetPlotter AI</h1>
          </div>
          <button 
            onClick={toggleTheme}
      ...
      <main className="flex-1 p-4 md:p-6 grid grid-cols-1 lg:grid-cols-12 gap-6 max-w-[1600px] mx-auto w-full">
        <aside className="lg:col-span-3 flex flex-col gap-6">
          <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-800 transition-colors">
            ...
            {state.isLoading ? (
              ...
              <p className="text-xs text-gray-600 dark:text-slate-400 leading-relaxed italic">{insights || "Visualizing spatial data from source."}</p>
            )}
          </div>
        ...
        <section className="lg:col-span-9 flex flex-col gap-6">
          {state.error && <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-400 p-4 rounded-xl text-sm shadow-sm">{state.error}</div>}
      ...
      <footer className="py-6 px-6 text-center text-[9px] text-gray-400 dark:text-slate-500 uppercase tracking-widest border-t dark:border-slate-800 mt-auto bg-white dark:bg-slate-900 transition-colors">
        Supports WGS84, European Decimals, and Combined Lat/Lng Columns.
      </footer>
    </div>
  );
};
